...
class Regularization:
    ...
    def loss_skew_symmetric(self, S, Sp, coeff=1.0):
        skew = self.skew_symmetric(S, Sp)
        reg = coeff * torch.mean(skew**2)
    ...
    def loss_gram_matrix(self, S, coeff=1.0):
        upper_sq_mat = coeff * torch.mean(
            torch.triu(self.gram_matrix(S), diagonal=1) ** 2
        )
    ...
    def loss_conjugacy_matrix(self, S, Sp, coeff=1.0):
        upper_sq_mat = coeff * torch.mean(
            torch.triu(self.conjugacy_matrix(S, Sp), diagonal=1) ** 2
        )
