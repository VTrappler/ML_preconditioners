...
class LimitedMemoryPrec(BaseModel):
    ...
    def inference(self, x: torch.Tensor, shift: float=1, data_norm: float=1.0) -> torch.Tensor:
        if not self.training:
            S, AS = self.forward_AS(torch.atleast_2d(x))
        return self.construct_LMP(S, AS, shift=shift) / data_norm
    
    def inverse_inference(self, x: torch.Tensor, shift: float=1.0, data_norm: float = 1.0)-> torch.Tensor:
        S, AS = self.forward_AS(torch.atleast_2d(x))
...
class LimitedMemoryPrecLinearOperator(LimitedMemoryPrec):
    ...
    def forward_linop(self, x: torch.Tensor) -> torch.Tensor:
        return self.layers_linear_op(x).reshape(len(x), self.state_dimension + 0, self.rank)
