...
from .utils import zero_module, normalization, count_flops_attn, gamma_embedding, pad
...
class EmbedSequential(nn.Sequential, EmbedBlock):
    ...
    def forward(self, x, emb: Optional[torch.Tensor]):
        for layer in self:
            # x = layer(x, emb)

            ##if isinstance(layer, EmbedBlock):
            if hasattr(layer, "_embed"):
                x = layer(x, emb)
            else:
                x = layer(x)
        return x
...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, out_channel=None):
        ...
        if use_conv:
            self.op = nn.Conv2d(self.channels, self.out_channel, 3, padding=1)
        else:
            self.op = nn.Identity()
...
class ResBlock(EmbedBlock):
    ...
    def forward(self, x, emb=torch.zeros(1)):  #: torch.Tensor=None):
        """
        ...
        if self.updown:
            ...
            h = in_rest[0].forward(x)
            for l in in_rest[1:-1]:
                h = l.forward(h)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.proj_out(h)
        return (x + h).reshape(b, c, width, height)
...
class UNet(nn.Module):
    ...
    def forward(self, x, gammas: torch.Tensor = torch.zeros(1)):
        """
